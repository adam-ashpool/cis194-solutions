...
skips :: [a] -> [[a]]
skips [] = []
skips xs = [[v | (i, v) <- zip [1..] xs, i `mod` n == 0] | n <- [1..(length xs)]]

localMaxima :: [Integer] -> [Integer]
localMaxima [] = []
localMaxima (x:y:z:xs) = if (x < y && z < y)
                         then y:(localMaxima (y:z:xs))
                         else localMaxima (y:z:xs)
localMaxima _ = []

drawLine :: [(Integer, Int)] -> Int -> String
drawLine cs h = [if ((snd (cs !! n)) >= h) then '*' else ' ' | n <- [0..9]] ++ "\n"

histogram :: [Int] -> String
histogram xs = let count c = length . filter (==c)
